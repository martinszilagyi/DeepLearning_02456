...
# NN Model
class Seq2SeqDeltaModelNoDecoderLSTM(nn.Module):
    def __init__(self,
        ...
        # Egy lineáris réteg az attention kimenet + előző predikció feldolgozására
        # Itt concat-oljuk az előző delta predikciót és a context vectort
        self.decoder_ff = nn.Sequential(
            nn.Linear(self.output_size + encoder_hidden_size, encoder_hidden_size),
            nn.ReLU(),
            nn.Linear(encoder_hidden_size, encoder_hidden_size),
            nn.ReLU(),
        )

        # Output réteg (delta pozíció predikció)
    ...
    def forward(self, x_dynamic, x_emb_nav_status, ground_truth, tf=0.8):
        ...
        device = last_known_pos.device

        # Kezdeti előző delta (nullával indulunk)
        prev_delta = torch.zeros(x_dynamic.size(0), self.output_size, device=device)

        # Scheduled sampling decay rate
        tf_decay_rate = tf / (self.forecast_steps + 200)

        for t in range(self.forecast_steps):
            # Attention, query: előző predikció helyett egy fix query-t használunk (pl. encoder utolsó hidden)
            query = h[-1]  # encoder végső rejtett állapota
            context_vector, attn_weights = self.attention(encoder_outputs, query, mask=mask)

            # Decoder input: concat of prev delta és attention context
            decoder_input = torch.cat([prev_delta, context_vector], dim=-1)
            hidden = self.decoder_ff(decoder_input)

            # Predikció
            pred_delta = self.fc_out(hidden)

            # Update position
            pos = pos + pred_delta
            abs_outputs.append(pos)

            # Teacher forcing: ground truth delta vs predikció váltogatása
            if torch.rand(1).item() < tf:
                if t == 0:
                    gt_delta_pos = ground_truth[:, 0, :2] - last_known_pos
                else:
                    gt_delta_pos = ground_truth[:, t, :2] - ground_truth[:, t - 1, :2]
                prev_delta = gt_delta_pos
            else:
...
def main():
    ...
    # Initialize model
    model = Seq2SeqDeltaModelNoDecoderLSTM(
        ...
        encoder_hidden_size=200,                                # Hidden size of encoder and decoder LSTMs (tuneable)
        output_size=2,                                          # Latitude, Longitude
