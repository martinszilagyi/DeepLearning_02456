...
# -------- UNIQUE PATHS FOR THE ENTIRE PROJECT --------
...
DYNAMIC_COLS = [
    ...
    'Navigational status_enum'     
]

NUM_DYNAMIC_FEATURES = len(DYNAMIC_COLS)  # = 12
...
RIVER_BOXES_DEG = [
    ...
    # Kiel Canal (south of the peninsula)
    {
        "name": "Kiel_Canal",
        "lat_min": 53.85,  # slightly south of Brunsbüttel
        "lat_max": 54.35,  # slightly north of Kiel
        "lon_min": 9.00,   # western entrance on the Elbe side
        "lon_max": 10.20,  # eastern entrance at Kiel-Holtenau
    },
]

RIVER_OVERSAMPLE_FACTOR = 4  # how many times more we want to see them
...
def load_normalization_info_from_parquets():
    """
    Reads normalization_info.json from the parquet folder
    and returns the original min/max of lat/lon.
    """
    info_path = os.path.join(PARQUET_DIR, "normalization_info.json")
    if not os.path.exists(info_path):
        raise FileNotFoundError(f"normalization_info.json not found in {info_path}")
...
# =========================
# NORMALIZED BOXES (same system as parquets)
# =========================
RIVER_BOXES = []
...
def is_river_sequence(df: pd.DataFrame) -> bool:
    """
    Returns True if a sequence is for a good part inside
    one of the boxes (in NORMALIZED coordinates).
    """
    if "Latitude" not in df.columns or "Longitude" not in df.columns:
        return False

    lat = df["Latitude"].values   # already normalized
    ...
    for box in RIVER_BOXES:
        ...
        # if at least 30% of the points are inside, we consider it "river"
        if mask.mean() > 0.3:
            return True
...
class Seq2SeqTrajectoryModel(nn.Module):
    """
    LSTM Encoder → Static Encoder → Autoregressive LSTM Decoder.
    Predicts future (lat, lon) step-by-step, enforcing more smoothness.
    """
    def __init__(self,
        ...
        for i, card in enumerate(cat_static_cardinalities):
            if card is None:
                raise ValueError(f"cat_static_cardinalities[{i}] is None, check num_ship_types / num_cargo_types")
            card_int = int(card)
            if card_int <= 0:
                raise ValueError(f"cat_static_cardinalities[{i}] = {card_int}, must be >= 1")
    ...
    def forward(self, x_dynamic, x_static_cats, x_static_nums):
        ...
        outputs = []
        # Initial step: we start from (0,0) in relative coordinates
...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # sequence length and split index
        seq_len = len(df)
        split_idx = int(seq_len * self.input_ratio)

        # reference point: LAST observed point (split_idx - 1)
        ref_idx = max(split_idx - 1, 0)
        lat0 = float(df.loc[ref_idx, 'Latitude'])
        lon0 = float(df.loc[ref_idx, 'Longitude'])

        # relative coordinates (in degrees, centered on the last known point)
        df['Lat_rel'] = df['Latitude'] - lat0
        df['Lon_rel'] = df['Longitude'] - lon0
        # --- Convert normalized angles to sin/cos ---

        # COG and Heading are normalized in [0,1], so convert to radians first
        df["COG_rad"] = df["COG"] * 2 * np.pi
        df["Heading_rad"] = df["Heading"] * 2 * np.pi
        ...
        # Multi-step target (still relative coordinates)
        y_future = df.loc[split_idx:split_idx + self.forecast_steps - 1,
                        ['Lat_rel', 'Lon_rel']]

        # If the sequence is too short, pad by repeating the last relative value
        ...
        for c in static_cat_cols:
            ...
            if pd.isna(val):
                ...
                idx = int(val)   # convert from float64 → int
...
def train_loop(model, dataloader, optimizer, device):
    ...
    for batch_idx, ((x_dynamic, x_static_cats, x_static_nums), y_target) in enumerate(dataloader):
        ...
        if True:  # DEBUG: print EVERY batch
...
def load_sequences(root):
    ...
    print(f"River oversampling: added {len(extra_sequences)} sequences")
    sequences.extend(extra_sequences)
    # ------------------------------------------------

    # 3) If we found nothing, stop BEFORE concatenating
    if len(sequences) == 0:
        raise RuntimeError("No parquet sequences found! Check PARQUET_DIR.")

    # 4) Compute cardinalities for embeddings
...
def split_data(sequences, val_ratio=0.10, test_ratio=0.10, random_state=1234):
    ...
    # val_ratio is intended as TOTAL percentage (10% of the total)
    # so here we rescale on the remaining (90%)
...
def evaluate_and_save_predictions(model, dataloader, device, output_json="predictions.json"):
    ...
    with torch.no_grad():
        ...
        for (x_dynamic, x_static_cats, x_static_nums), y_target in dataloader:
            ...
            # Move everything to CPU as numpy
            ...
            for i in range(len(y_pred_np)):
                ...
                # --- FIX: remove possible NaNs from target and prediction ---
                ...
                # ---- ALIGNMENT OF STARTING POINT ----
                # anchor = last known point (last blue)
                anchor = known_abs_norm[-1]  # shape (2,)

                # shift GT and pred so that their first point matches anchor
...
def main():
    ...
    # Always load parquets from the same shared directory
    root = PARQUET_DIR
    ...
    # filter out only sequences that are too short, without further cleaning
